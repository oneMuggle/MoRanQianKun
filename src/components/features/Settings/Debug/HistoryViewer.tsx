...
const HistoryViewer: React.FC<Props> = ({ history = [], memorySystem }) => {
    ...
    const handleQuickExport = () => {
        ...
        快速导出为Md(history, undefined, '对话记录');
    };
    ...
    return (
        <div className="h-full flex flex-col animate-fadeIn">
            <div className="flex items-center justify-between mb-4 shrink-0">
                <h3 className="text-wuxia-gold font-serif font-bold text-lg">互动历史存档</h3>
                <div className="flex items-center gap-2">
                    <button
                        onClick={handleQuickExport}
                        disabled={history.length === 0}
                        className="px-3 py-1.5 text-xs rounded-lg border border-wuxia-gold/30 bg-black/40 text-wuxia-gold hover:bg-wuxia-gold/10 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                        title="快速导出为 Markdown"
                    >
                        导出
                    </button>
                    <button
                        onClick={() => setShowExportPanel(prev => !prev)}
                        className="px-3 py-1.5 text-xs rounded-lg border border-gray-600 bg-black/40 text-gray-300 hover:border-gray-400 transition-colors"
                    >
                        {showExportPanel ? '收起' : '更多格式'}
                    </button>
                </div>
            </div>

            {showExportPanel && (
                <div className="shrink-0 mb-3 p-3 border border-gray-700 rounded-lg bg-black/30">
