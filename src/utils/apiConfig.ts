...
export const 获取剧情回忆接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: recallBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: recallBaseUrl || current.baseUrl, apiKey: recallApiKey || current.apiKey, model: recallModel };
};
...
export const 获取记忆总结接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: summaryBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: summaryBaseUrl || current.baseUrl, apiKey: summaryApiKey || current.apiKey, model: summaryModel };
};
...
export const 获取文章优化接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: polishBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: polishBaseUrl || current.baseUrl, apiKey: polishApiKey || current.apiKey, model: polishModel };
};

export const 获取设备消息接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    const current = 获取当前接口配置(settings);
    if (!current) return null;
    const feature = (settings as any)?.功能模型占位;
    const independent = Boolean(feature?.设备消息独立模型开关);
    if (!independent) return current;
    const deviceModel = 读取字符串(feature?.设备消息使用模型).trim();
    if (!deviceModel) return current;
    const deviceBaseUrl = 读取字符串(feature?.设备消息API地址).trim();
    const deviceApiKey = 读取字符串(feature?.设备消息API密钥).trim();
    const supplier = deviceBaseUrl ? 推断供应商(deviceBaseUrl) : current.供应商;
    return { ...current, 供应商: supplier, 协议覆盖: deviceBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: deviceBaseUrl || current.baseUrl, apiKey: deviceApiKey || current.apiKey, model: deviceModel };
};
...
export const 获取变量计算接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: variableBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: variableBaseUrl || current.baseUrl, apiKey: variableApiKey || current.apiKey, model: variableModel };
};
...
export const 获取世界演变接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: worldBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: worldBaseUrl || current.baseUrl, apiKey: worldApiKey || current.apiKey, model: worldModel };
};
...
export const 获取小说拆分接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (baseUrl && apiKey && dedicatedModel) {
        return { id: current?.id || 'novel_decomposition_dedicated', 名称: current?.名称 || '小说拆分独立接口', 供应商: 推断供应商(baseUrl), 协议覆盖: 'auto', baseUrl, apiKey, model: dedicatedModel, maxTokens: 小说拆分最大输出Token };
    }
    if (!current) return null;
    ...
    return { ...current, 供应商: supplier, 协议覆盖: baseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: baseUrl || current.baseUrl, apiKey: apiKey || current.apiKey, model, maxTokens: 小说拆分最大输出Token };
};
...
export const 获取规划分析接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    const supplier = baseUrl ? 推断供应商(baseUrl) : current.供应商;
    return { ...current, 供应商: supplier, 协议覆盖: baseUrl ? 'auto' : (current.协议覆盖 || 'auto'), baseUrl: baseUrl || current.baseUrl, apiKey: apiKey || current.apiKey, model };
};
...
export const 获取文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (Array.isArray(imgConfigs) && imgConfigs.length > 0) {
        ...
        return {
            ...current, 供应商: resolvedBaseUrl ? 推断供应商(resolvedBaseUrl) : current.供应商, 协议覆盖: baseUrl ? 'auto' : (current.协议覆盖 || 'auto'),
    ...
    return {
        ...current, 供应商: supplier, 协议覆盖: imageBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'),
...
export const 获取场景文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return {
        ...sharedConfig, 供应商: supplier, 协议覆盖: sceneBaseUrl ? 'auto' : (canReuseSharedConnection ? (sharedConfig.协议覆盖 || 'auto') : 'auto'),
...
export const 获取生图词组转化器接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return {
        ...current, 供应商: supplier, 协议覆盖: transformerBaseUrl ? 'auto' : (current.协议覆盖 || 'auto'),
