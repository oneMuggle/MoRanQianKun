...
/** 服装层次损坏程度的视觉颜色标识 */
export const 服装状态颜色映射: Record<string, string> = {
    '完好': 'text-green-400',
    '褶皱': 'text-yellow-300',
    '凌乱': 'text-yellow-400',
    '破损': 'text-orange-400',
...
/** 服装层次的热度值 (0-1) */
export const 服装状态热度映射: Record<string, number> = {
    '完好': 1,
    '褶皱': 0.8,
    '凌乱': 0.6,
    '破损': 0.4,
...
/** 从服装层次条目派出的 UI 状态标识 */
type 层次UI状态 = '穿着' | '半敞' | '褪下' | '移除';

/** 将损坏程度映射为 UI 状态 */
function 损坏程度转UI状态(损坏程度: string): 层次UI状态 {
    if (损坏程度 === '移除') return '移除';
    if (损坏程度 === '完好') return '穿着';
    return '半敞';
}
...
/**
 * 从 NPC 的服装层次推导某个部位的状态
 */
function 从层次获取状态(部位: 服饰部位分类, npc: NPC结构): 层次UI状态 | undefined {
    const 层次 = npc.完整演化状态?.服装层次?.层次;
    if (!层次) return undefined;
    // 取最外层未移除的条目
    const 条目 = 层次.find(e => e.部位 === 部位 && e.损坏程度 !== '移除');
    if (!条目) return undefined;
    return 损坏程度转UI状态(条目.损坏程度);
}
 ...
 */
export const 获取部位层次摘要 = (npc: NPC结构): Record<服饰部位分类, { 有无服装: boolean; 状态: 层次UI状态 | undefined; 名称?: string }> => {
    const 档案 = npc.服饰档案;
    const 摘要: Record<服饰部位分类, { 有无服装: boolean; 状态: 层次UI状态 | undefined; 名称?: string }> = {} as any;
