...
- [x] 步骤 1：实现开局地图保底数据（P0-4.1）
- [x] 步骤 2：实现旅行事件注入主叙事流（P0-4.2）
- [x] 步骤 3：消除 MapModal 中的 `any` 类型（P1-4.3）
- [x] 步骤 4：实现旅行事件自动清理（P1-4.4）
- [x] 步骤 5：统一手机 MapApp 风格（P2-4.5）
- [x] 步骤 6：地图搜索/筛选功能（P2-4.8）
...
| 步骤 | 变更文件 | 说明 |
|------|----------|------|
| 4.1 | `openingStoryWorkflow.ts` | 开局后检查 `世界.地图` 是否为空，若为空则根据环境信息自动生成保底地图/建筑 |
| 4.2 | `useTravelAndTrade.ts`, `utilityDomain.ts` | 旅行成功后将事件描述作为系统消息写入 `state.历史记录` |
| 4.3 | `MapModal.tsx`, `MobileMapModal.tsx` | 约 12 处 `any` 替换为 `地图结构` / `建筑结构` |
| 4.4 | `useTravelAndTrade.ts` | `handleTravel` 执行前先清空 `旅行事件列表` |
| 4.5 | `MapApp.tsx` | 所有 emoji 替换为内联 SVG 图标，与项目武侠美术风格对齐 |
| 4.8 | `MapModal.tsx`, `MobileMapModal.tsx` | 桌面端和移动端均添加搜索框，支持按地图名称和地点层级模糊搜索，过滤结果实时更新列表 |
