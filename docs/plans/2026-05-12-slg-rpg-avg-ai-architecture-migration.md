...
**验收标准**：
- [x] EventBus 发布事件后所有订阅者收到通知
- [x] 约束构建器生成 XML 覆盖 SLG/RPG/AVG 三层状态
- [x] 与现有 EngineRegistry 集成，引擎自动注册/注销事件订阅
- [x] `npm run build` 通过
...
**验收标准**：
- [x] useGame.ts 从 3000 行拆分为 12+ 个子模块
- [x] 所有引擎（含扩展玩法）注册到 EngineRegistry 正确工作
- [x] 事件总线跨引擎通信正常
- [x] 集成测试全部通过
- [x] `npm run build` 通过
...
- [x] 11.5 build 通过 — TypeScript 编译无错误
...
- [x] 12.5 CG 图鉴管理 — 内置在 `endingResolver.ts` 中 (`CGManager`)，UI 展示待前端实施
