...
在现代纪元（`contemporary_campus`）的"校园纪元"子纪元下，深化手机系统的功能，包括论坛系统（校园BBS）、聊天系统（类微信）、以及其他校园特有APP。系统需支持 SFW 和 NSFW 双模式内容切换。
...
- 当前已实现的基础APP：MapApp、ContactsApp、ForumApp、NewsApp、AlbumApp（均为空壳）
- 校园纪元设备配置（`smartphone`）已存在
...
3. **其他APP**：课程表、校园卡、社团活动等校园特有APP
4. **SFW/NSFW 双模式**：内容根据游戏配置切换，论坛/聊天中可出现不同尺度的内容
5. **设备配置**：为 `contemporary_campus` 添加专属设备配置和里模式覆盖
...
### 6. 里模式增强
- 论坛：分类名变为"深夜树洞"、"暗面灌水"等
- 私聊：增加"加密频道"，显示更亲密的对话
- 课程表：增加"秘密约会"条目
- 颜色主题：使用 `liMode.themeColor` 切换
...
### Phase 1：数据模型与设备配置（2文件，低风险）

#### 步骤 1：创建校园手机数据模型
- **文件**：`models/campusPhone.ts`
- **内容**：定义 `论坛帖子`、`论坛回复`、`私聊会话`、`聊天消息`、`课程表`、`课程`、`校园卡`、`消费记录`、`社团活动` 等类型
- **依赖**：无
- **风险**：低

#### 步骤 2：扩展 MobileApp 类型
- **文件**：`models/mobileDevice.ts`
- **内容**：在 `MobileApp` 类型中添加 `'schedule' | 'campus_card' | 'club' | 'confession'`；在 `DeviceMessage.type` 中添加对应类型
- **依赖**：步骤 1
- **风险**：低
...
- **内容**：在 `eraDeviceConfigs` 中添加 `'contemporary_campus'` 配置：
  - 设备名：`智能手机`
  - 应用列表：`['map', 'contacts', 'chat', 'forum', 'news', 'album', 'tools', 'schedule', 'campus_card', 'club', 'confession']`
  - 正常模式应用名：地图、通讯录、私聊、校园论坛、校园资讯、相册、工具、课程表、校园卡、社团活动、表白墙
  - 里模式覆盖应用名：夜行地图、关系网、私密聊天、深夜树洞、暗面推送、私密相册、暗面工具、秘密约会、校园钱包、地下社团、匿名告白
  - 里模式主题色：使用 `contemporary_campus` 的 `liMode.themeColor`（绿色变体）
- **依赖**：步骤 2
- **风险**：低

### Phase 2：APP组件开发（5文件，中风险）

#### 步骤 4：开发 CampusForumApp
...
#### 步骤 8：开发 CampusClubApp
- **文件**：`components/features/MobileDevice/apps/CampusCampusClubApp.tsx`
- **内容**：
  - 社团活动卡片列表
...
### Phase 3：集成与NSFW适配（3文件，中高风险）

#### 步骤 9：更新 MobileHome 组件
- **文件**：`components/features/MobileDevice/MobileHome.tsx`
- **内容**：
  - 在 `appIcons` 中添加新APP的图标
  - 在 `renderActiveApp` 中添加新APP的路由
...
- **内容**：
  - 在 `contemporary_campus` 节点的 `liMode` 中添加结构化字段：
    - `stageRules`（平然/羞耻/欲望 三阶段行为规则）
    - `intensityLevels`（微暗/暧昧/露骨 三级强度定义，手机场景特化）
    - `sceneTypes`（增加手机场景：深夜私聊、论坛发帖、课程表约会、校园卡消费记录）
  - 在 `uiCopy` 中添加手机相关文案
- **依赖**：步骤 9
- **风险**：中

#### 步骤 11：SFW/NSFW 内容切换适配
- **文件**：多个APP组件
- **内容**：
...
5. 在设置中切换 NSFW 等级（无/点到为止/适度展开/完全展开），验证内容变化
6. 测试里模式强度切换（微暗/暧昧/露骨）
7. 测试 SFW/NSFW 来回切换，确保无内容残留
...
- [x] Phase 1：数据模型与设备配置 — 计划编写完成
- [ ] Phase 1：实施完成
- [ ] Phase 2：APP组件开发 — 实施完成
- [ ] Phase 3：集成与NSFW适配 — 实施完成
- [ ] 全部测试通过，功能完成
