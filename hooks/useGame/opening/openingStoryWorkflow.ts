    ...
    try {
        ...
        const openingNSFWAppearancePrompt = openingGameConfig.启用NSFW模式 === true
            ? [
                ...
                '',
                '【NSFW社交档案生成要求】',
                '- 初始化社交NPC时，必须为每个具名NPC建立完整的NSFW档案。',
                '- 在 `<变量规划>` 的社交对象描述中，为每个NPC补充以下NSFW字段：',
                '  - 性癖/性癖倾向：根据NPC的身份、年龄、性格和背景合理推断，可包含偏好类型和强度等级。',
                '  - 敏感点/敏感区域：分条目列举 2~4 个敏感点，每个条目写清：部位名称、敏感度等级（1~5）、开发程度（未开发/初步探索/渐入佳境/深度开发/完全开发）、被触碰时的反应描述。',
                '  - 衣着风格：描述NPC日常穿着的偏好、风格、常穿内衣类型与颜色，应体现NPC的性癖、性格、经历的内在关联。',
                '  - 服饰档案：按部位（上衣/下着/内衣/内裤/袜子/鞋子/配饰等）分别列出NPC当前穿着的每件具体衣物，每个部位写清名称与描述。系统会自动从服饰档案同步到服装层次。',
                '  - 穿着风格演变趋势：当NPC的性癖/经历/人格发生变化时，穿着风格可能如何演变。',
                '  - NSFW行为特征：在亲密互动中可能展现的倾向或反应模式。',
                '- 这些NSFW字段应自然融入NPC的整体人设，不要生硬堆砌。'
            ].join('\n')
            : '';
        ...
        if (!hasOpeningCommands) {
            ...
            deps.设置社交(deps.规范化社交列表(openingStateAfterCommands.社交, { eraId: options?.eraId }));
            deps.设置世界(世界数据);
