...
/** 从上下文快照参数提取校园NSFW运行时参数 */
function 构建上下文校园NSFW参数(params: 构建上下文快照参数) {
    const 欲望系统 = params.校园系统?.欲望系统;
    if (!欲望系统?.NPC欲望档案) return undefined;
    ...
    return {
        ...
        内容强度: params.gameConfig?.校园NSFW设置?.NSFW内容强度,
    };
}
...
export const 构建上下文快照数据 = async (params: 构建上下文快照参数): Promise<上下文快照> => {
    ...
    const builtContext = params.构建系统提示词({
        promptPool: params.prompts,
        memoryData: normalizedMem,
        socialData: params.社交,
        statePayload: {
            角色: params.角色,
    ...
    const { messageEntries } = 构建主剧情请求参数({
        ...
        worldbooks: params.世界书列表,
        校园NSFW参数: 构建上下文校园NSFW参数(params),
        时代配置ID: params.时代配置ID
    });
