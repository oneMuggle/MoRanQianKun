...
export const 应用写真系统状态更新 = (
  ...
  if (更新.更新拍摄项目) {
    ...
    for (const raw更新 of 更新.更新拍摄项目 as any[]) {
      ...
      // Fallback：如果无 id 或匹配失败，基于 模特Id + 摄影师Id + 活跃状态 匹配
      if (已有索引 < 0) {
        const 模特Id = raw更新.模特Id;
        const 摄影师Id = raw更新.摄影师Id || raw更新.摄影师ID;
        if (模特Id && 摄影师Id) {
          已有索引 = 新项目列表.findIndex(
            (p: any) => p.模特Id === 模特Id && p.摄影师Id === 摄影师Id && p.拍摄阶段 !== '已完成' && p.拍摄阶段 !== '已取消' && p.拍摄阶段 !== '已投诉'
          );
        }
      }
      ...
      if (已有索引 >= 0) {
        ...
        新项目列表[已有索引] = {
          ...现有,
          ...raw更新,
          id: 现有.id || 项目ID || `shoot_${Date.now()}`,
        };
      } else {
        // 新项目：创建完整最小项目
        const 最终ID = 项目ID || `shoot_${Date.now()}`;
