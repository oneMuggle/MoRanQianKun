...
export const 构建NPC_NSWF卡片 = (
  ...
  // 服装状态 + 服饰名称
  if (npc.当前服装状态 || npc.服饰档案) {
    const 服装 = npc.当前服装状态;
    const 档案 = npc.服饰档案;
    const 服装摘要: string[] = [];
    const 部位映射 = [
      { 状态键: '上衣状态' as const, 档案键: '上衣' as const, 标签: '上衣' },
      { 状态键: '下装状态' as const, 档案键: '下着' as const, 标签: '下装' },
      { 状态键: '内衣状态' as const, 档案键: '内衣' as const, 标签: '内衣' },
      { 状态键: '内裤状态' as const, 档案键: '内裤' as const, 标签: '内裤' },
      { 状态键: '袜饰状态' as const, 档案键: '袜子' as const, 标签: '袜饰' },
    ] as const;
    for (const m of 部位映射) {
      const 状态值 = 服装?.[m.状态键];
      const 档案值 = 档案?.[m.档案键];
      if (档案值 && 状态值) {
        服装摘要.push(`${m.标签}[${档案值.名称}](${状态值})`);
      } else if (档案值) {
