...
import { ConfirmOptions } from './components/ui/InAppConfirmModal';
import { useGame } from './hooks/useGame';
import { useResponsive } from './hooks/useResponsive';
import { useModalOpeners } from './hooks/useModalOpeners';
...
import { 全部时代配置, 获取时代主题方案 } from './models/system';
...
const App: React.FC = () => {
    ...
    // Modal openers — extracted to hooks/useModalOpeners.ts
    const modalStates = {
    ...
    return (
        <MusicProvider visualConfig={state.visualConfig} onSaveVisual={actions.saveVisualSettings}>
            ...
            {state.view === 'game' && (
                ...
                <div className={`relative flex-1 flex flex-col w-full h-full rounded-2xl overflow-hidden bg-ink-black shadow-2xl ${state.gameConfig?.启用里武侠模式 ? 'lixia-active' : ''} ${state.gameConfig?.启用里志怪模式 ? 'lizhiguai-active' : ''}`}>

                    {/* 里武侠花瓣装饰层 */}
                    {state.gameConfig?.启用里武侠模式 && <div className="lixia-petals" aria-hidden="true" />}

                    {/* 里志怪鬼火粒子装饰层 */}
                    {state.gameConfig?.启用里志怪模式 && <div className="lizhiguai-particles" aria-hidden="true" />}
                    ...
                    <div className="flex-1 flex overflow-hidden relative z-10 mx-1 mb-1">
                        ...
                        {/* 左侧栏 */}
                        <div className={`hidden md:block w-[14.285714%] h-full relative z-20 bg-ink-black/95 border-r border-wuxia-gold/20 flex flex-col shadow-[10px_0_20px_rgba(0,0,0,0.5)] lixia-panel-left ${state.gameConfig?.启用里志怪模式 ? 'lizhiguai-panel-left' : ''}`}>
                            <LeftPanel
                                角色={state.角色}
                        ...
                        {/* 右侧栏 */}
                        <div className={`hidden md:block w-[14.285714%] h-full relative z-20 bg-ink-black/95 border-l border-wuxia-gold/20 flex flex-col shadow-[-10px_0_20px_rgba(0,0,0,0.5)] lixia-panel-right ${state.gameConfig?.启用里志怪模式 ? 'lizhiguai-panel-right' : ''}`}>
                            <RightPanel 
            ...
            {state.showSettings && (
                <懒加载边界>
                    <SettingsPanel
                        ...
                        currentEra={state.currentEra}
                        eraInfo={meta.eraInfo}
                        eraTheme={state.currentEra ? 获取时代主题方案(state.currentEra) : undefined}
                        availableEras={全部时代配置}
                        onEraChange={actions.handleEraChange}
                        history={state.历史记录}
