...
const 标准化功能模型占位 = (raw: any): 功能模型占位配置结构 => {
    ...
    return {
        ...
        图片后端注册表地址: 读取字符串(raw?.图片后端注册表地址).trim(),
        当前图片后端发现ID: 读取字符串(raw?.当前图片后端发现ID).trim(),
        当前场景图片后端发现ID: 读取字符串(raw?.当前场景图片后端发现ID).trim()
    };
};
...
export const 获取文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    // 如果配置了发现的后端，优先使用发现的后端 URL
    const discoveredId = 读取字符串(feature?.当前图片后端发现ID).trim();
    const discoveredUrl = discoveredId && feature?.图片后端注册表地址
        ? `${读取字符串(feature.图片后端注册表地址).replace(/\/api\/image-backend\/cnb-sync.*/, '')}`
        : '';
    const 图片后端可复用主接口地址 = 图片后端类型 === 'openai' || 图片后端类型 === 'novelai';
    const resolvedImageBaseUrl = imageBaseUrl || discoveredUrl || (图片后端可复用主接口地址 ? current.baseUrl : '');
...
return {
    ...
    // 如果配置了场景专属发现的后端，优先使用
    const sceneDiscoveredId = 读取字符串(feature?.当前场景图片后端发现ID).trim();
    const sceneDiscoveredUrl = sceneDiscoveredId && feature?.图片后端注册表地址
        ? `${读取字符串(feature.图片后端注册表地址).replace(/\/api\/image-backend\/cnb-sync.*/, '')}`
        : '';
    const canReuseSharedConnection = sceneBackend === sharedBackend;
    const resolvedBaseUrl = sceneBaseUrl || sceneDiscoveredUrl || (canReuseSharedConnection ? sharedConfig.baseUrl : '');
