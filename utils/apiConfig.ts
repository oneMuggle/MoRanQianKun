...
/** CNB ComfyUI 地址 */
const 解析CnbComfyui地址 = (url: string): string => {
    if (!url?.trim()) return '';
...
export const 获取文生图接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    if (Array.isArray(imgConfigs) && imgConfigs.length > 0) {
        ...
        const resolvedBaseUrl = isCnbComfyui
            ? (cnbBaseUrl || baseUrl)
            : (baseUrl || (backend === 'openai' || backend === 'novelai' ? current.baseUrl : ''));
    ...
    // CNB ComfyUI 地址（fallback 路径）
    const cnbUrl = 读取字符串(feature?.cnbComfyui地址).trim();
    const cnbBaseUrl = 解析CnbComfyui地址(cnbUrl);
    const isCnbComfyuiFallback = 图片后端类型 === 'comfyui' && cnbBaseUrl;
    const 图片后端可复用主接口地址 = 图片后端类型 === 'openai' || 图片后端类型 === 'novelai';
    const resolvedImageBaseUrl = isCnbComfyuiFallback
        ? (cnbBaseUrl || imageBaseUrl || (图片后端可复用主接口地址 ? current.baseUrl : ''))
...
return {
    ...
    const isSceneCnbComfyui = sceneBackend === 'comfyui' && sceneCnbBaseUrl;
    const canReuseSharedConnection = sceneBackend === sharedBackend;
    const resolvedBaseUrl = isSceneCnbComfyui
        ? (sceneCnbBaseUrl || sceneBaseUrl || (canReuseSharedConnection ? sharedConfig.baseUrl : ''))
