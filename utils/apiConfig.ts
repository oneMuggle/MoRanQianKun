...
export const 获取设备消息接口配置 = (settings: 接口设置结构): 当前可用接口结构 | null => {
    ...
    return { ...current, 供应商: supplier, 协议覆盖: deviceBaseUrl ? 'auto' : current.协议覆盖, baseUrl: deviceBaseUrl || current.baseUrl, apiKey: deviceApiKey || current.apiKey, model: deviceModel };
};
