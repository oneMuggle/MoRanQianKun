...
const CampusForumApp: React.FC<AppProps> = ({ eraId, mode, appId, onBack, gameContext }) => {
    ...
    const posts: ForumPost[] = useMemo(() => {
        ...
        if (gameContext?.世界?.进行中事件) {
            gameContext.世界.进行中事件.forEach((event, idx) => {
                result.push({
                    id: `event-${idx}`,
                    author: event.关联人物?.[0] || '匿名',
                    title: event.事件名,
                    content: event.事件说明,
                    category: '校园资讯',
                    time: event.开始时间 || '近日',
                    ...
                    replyList: event.关联人物.slice(1, 4).map(p => ({
                        author: p,
                        content: '听说这事了，确实如此。',
                        time: event.开始时间 || '近日',
                    })),
        ...
        if (world?.世界镜头规划) {
            world.世界镜头规划.slice(-3).forEach((lens, idx) => {
                result.push({
                    id: `lens-${idx}`,
                    author: lens.关联人物?.[0] || '路人甲',
                    title: lens.镜头标题,
                    content: lens.镜头内容 || '',
                    category: '情感树洞',
                    time: lens.触发时间 || '近日',
            ...
            <div className="flex flex-col h-full">
                <div className="flex items-center gap-3 px-4 py-3 border-b border-gray-700/50">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <span className="text-[10px] text-gray-500">{selectedPost.author} &middot; {selectedPost.time}</span>
                    </div>
                </div>
                <div className="flex-1 overflow-y-auto p-4 space-y-4">
                    <div className="rounded-lg bg-gray-800/40 border border-gray-700/30 p-3">
                        <div className="flex items-center justify-between mb-2">
                            <span className="text-[10px] text-blue-400/60 bg-blue-400/10 px-1.5 py-0.5 rounded">{selectedPost.category}</span>
                                ...
                                {selectedPost.replyList.map((reply, idx) => (
                                    <div key={idx} className="rounded-lg bg-gray-900/40 border border-gray-800/30 p-3">
                                        <div className="flex items-center justify-between mb-1">
                                            <span className="text-xs text-blue-300/70">{reply.author}</span>
                            ...
                            <li key={post.id}>
                                ...
                                >
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="text-[10px] text-blue-400/60 bg-blue-400/10 px-1.5 py-0.5 rounded">{post.category}</span>
